...
> 如果说 ClickHouse 想把自己的生产推荐路径越收越拢，那么 Keeper 就是那条路径里最有代表性的一个组件。

## 在这个 wiki 中的重要性

Keeper 让我把 ClickHouse 的“高可用”理解得更具体了。很多时候我们说数据库集群有副本、有 `ON CLUSTER`、有分布式表，但这些能力之所以能稳定运作，背后都依赖一个明确的协调层。没有这层，很多看起来像“数据库功能”的东西其实都只是表面现象。
...
Keeper 最重要的边界，是**兼容不等于完全等价**。外部生态、混合集群、动态重配置等问题，都需要按 Keeper 自己的文档来判断，而不是默认套用 ZooKeeper 的全部经验。我会把这看成它最容易被误判、也最该被提前讲清楚的地方。
