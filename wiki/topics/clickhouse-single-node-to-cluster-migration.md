...
后来我又把这条短路径，进一步套进一个更贴近真实业务的验证里：目标不是示例表，而是 SwanLab 这种“单实例起步、三张核心表、目标拓扑直接上 `4 × 2`、新增写入走 `Vector` 双写、目标侧禁止自动建表”的组合。那次验证最后同样跑通了，说明这条短路径并不只适合 demo，而是可以自然扩展成更接近生产的迁移骨架。
