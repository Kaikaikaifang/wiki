...
2. 批次维度统一使用 `date x target_shard`，必要时再带 `projectId` / `experimentId`；
3. 导出阶段预先计算目标 shard；
4. 导入阶段直写对应 shard 的 `*_local`；
5. 第二副本交给复制追平；
...
如果业务硬性要求全历史先入新集群，那就不要继续加长当前小时级脚本，而是直接升级成统一的 shard-aware 回灌平台：
...
我现在会把这三项作为最优先升级：按 shard 直写 `*_local`、多快照克隆并行导出、`date x shard + 自动 split`。它们共同把当前已经验证可行的数据管道，升级成能承受生产窗口的统一执行系统。
