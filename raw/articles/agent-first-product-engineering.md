...
source: "https://posthog.com/newsletter/agent-first-product-engineering"
author:
published:
created: 2026-06-13
description: "Five principles for building products that work well for AI agents and MCP servers."
tags:
  - "clippings"
---
![The golden rules of agent-first product engineering](https://res.cloudinary.com/dmukukwp6/image/upload/f_webp,h_1080,w_1871,c_fill,g_auto,q_auto/Untitled_January_22_2026_10_34_32_copy_6_1_c5abc36b7f?_a=AXAH4nI0)
...
Agents as a new interaction layer between your product and users
...
For example, let's say you ask an agent to set up an A/B test for a new pricing page. The agent would use [the PostHog MCP](https://posthog.com/docs/model-context-protocol) to [create the feature flag](https://posthog.com/feature-flags), [create the experiment](https://posthog.com/experiments), link it to the flag it just created, and set up the [insight](https://posthog.com/docs/product-analytics/insights) and metrics.
...
So in our MCP v2, we let agents query PostHog data in a language they already speak fluently: [SQL](https://posthog.com/sql).
...
That works for general-purpose agents since they're designed to be flexible. Anthropic can't predict everything that everyone is [using Claude for](https://posthog.com/newsletter/ai-coding-mistakes).
...
So for [product engineers](https://posthog.com/product-engineer/what-is-a-product-engineer), you can strike that balance by front-loading what you know every session will need, and defer the rest.
...
- **PostHog-specific taxonomy.** What's a feature flag, experiment, [session replay](https://posthog.com/session-replay), etc.
- **Our [SQL](https://posthog.com/docs/sql) syntax.** How to use our custom translation layer over ClickHouse SQL.
...
That's [what good skills look like](https://posthog.com/handbook/engineering/ai/writing-skills).
...
- **[Taste](https://posthog.com/newsletter/good-taste-great-products) and craft.** Not just how to use your product correctly, but how to use it well.
...
> For [activation](https://posthog.com/newsletter/wtf-is-activation) and [retention](https://posthog.com/docs/product-analytics/retention) events, use the `$pageview` event by default. Avoid infrequent or inconsistent events like `signed in` unless asked explicitly, as they skew the data.
...
In traditional software, even if user behavior isn't predictable, the code is. With AI, you lose that stability; the same inputs [won't produce the same outputs anymore](https://posthog.com/blog/correct-llm-code-generation).

This means [classic testing methods will leave a gap](https://posthog.com/blog/testing-ai-agents), so you need new ways to catch the things automated tests no longer will.

You need to treat the agent like you would treat a user. [Talk to them](https://posthog.com/newsletter/talk-to-users), build empathy for them, and develop an intuition of what they want.
...
Dogfooding headlessly via the CLI
