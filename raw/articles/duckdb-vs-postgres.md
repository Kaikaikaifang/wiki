...
source: "https://posthog.com/blog/duckdb-vs-postgres"
author:
published:
created: 2026-06-13
description: "We've made  some apples to oranges comparisons  over the years. None have been so diametrically opposing than Postgres and DuckDB. However, that also…"
tags:
  - "clippings"
---
![In-depth: DuckDB vs Postgres](https://res.cloudinary.com/dmukukwp6/image/upload/f_webp,h_1080,w_1920,c_fill,g_auto,q_auto/duckhog_1c2ac8c463?_a=AXAH4nI0)

We've made [some apples to oranges comparisons](https://posthog.com/blog/clickhouse-vs-postgres) over the years. None have been so diametrically opposing than Postgres and DuckDB. However, that also makes for a neat pairing: comparing Postgres and DuckDB is a masterclass on how databases are layers upon layers of design decisions. At *almost* every layer, Postgres and DuckDB go separate paths.
...
DuckDB single-node scaling diagram
...
The second is ClickHouse. We adopted ClickHouse when Postgres struggled to handle our users' analytical requirements. [ClickHouse is an OLAP database like DuckDB](https://posthog.com/blog/duckdb-vs-clickhouse), but has a similar client-server set-up as Postgres. It's blazingly-fast and can be tuned for repeat queries. ClickHouse is also incredibly scalable, being able to scale horizontally and vertically. This makes ClickHouse an ideal home for our customers' hot analytical data that's routinely accessed in the PostHog app.

And finally, there's DuckDB. We integrated DuckDB when we launched our [data warehouse product](https://posthog.com/data-stack/managed-warehouse), which is effectively a warehouse for [all of your PostHog (and other) data](https://posthog.com/blog/data-warehouse-at-posthog). To make it easy for our users to query their warehouse, we set-up a system that uses DuckDB as a query engine. And given that many analytics tools are used to connecting to Postgres-like databases, we decided to build [Duckgres](https://github.com/PostHog/duckgres), a DuckDB wrapper that makes it Postgres Wire Protocol compatible. Like our other products, it's open source and perfect for any DuckDB deployment that needs compatibility with [BI tooling](https://posthog.com/data-stack/business-intelligence).
