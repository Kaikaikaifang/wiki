...
source: "https://posthog.com/blog/duckdb-vs-clickhouse"
author:
published: 2026-05-01
created: 2026-06-13
description: "ClickHouse and DuckDB are two heavyweights in the OLAP category. This makes them appear as direct competitors. In a few respects they are, but they…"
tags:
  - "clippings"
---
![DuckDB vs ClickHouse: Why we use both at PostHog](https://res.cloudinary.com/dmukukwp6/image/upload/f_webp,h_1080,w_1920,c_fill,g_auto,q_auto/duckhog_1c2ac8c463?_a=AXAH4nI0)
...
Previously, I'd likened the debate between ClickHouse and [Postgres](https://posthog.com/blog/duckdb-vs-postgres) to that [of grapefruits and grapes](https://posthog.com/blog/clickhouse-vs-postgres). While the difference between ClickHouse and DuckDB isn't as drastic, it's similar to comparing onions and spring onions: they share a lot in common, but are used for fundamentally different things.

To provide a high-level "say less" tl;dr (or whatever kids say these days): ClickHouse is a feature-packed, highly-configurable database for fast analytical queries on massive data scaled across multiple instances. DuckDB, meanwhile, is an embeddable, single-node engine that shines in two modes: as a query engine for fast exploratory analytics directly over files (CSVs, Parquet, etc.) or as the " [SQLite of OLAP](https://posthog.com/blog/duckdb-vs-sqlite) ", a lightweight analytical database for datasets that can fit on a single machine.
...
ClickHouse horizontal scaling diagram
...
PostHog uses three databases. For PostHog's application state, we use Postgres in all of the ways you'd expect. In fact, at our start, we exclusively used Postgres until [our analytical queries were frying it alive](https://posthog.com/blog/how-we-turned-clickhouse-into-our-eventmansion). We then added ClickHouse, a real-time analytical engine that powers our funnels, trends, retention, and paths over billions of events.
